# Phase 11: Physiology, Development, And Life History
...
culture stack needs. Paying that cost through Phases 6 to 10 would reduce
...
being rewritten anyway, happens in Phase 7. Doing it twice would be strictly
worse. Physiology realism, which is separable, happens here, after the
scale-sensitive culture experiments have run.

The cost is accepted and stated: **results from Phases 6 to 10 do not
...
- Sexual selection: mate choice conditioned on perceived phenotype through
  the Phase 9 perception channels, replacing compatibility-threshold-only
  pairing.
...
- Phase 10, so the campaigns whose results this phase invalidates have
  already been run.
- Phase 9 perception channels for mate choice.
...
- **B**: Phase 10 abstract energetics.
- **M-high / M-low**: an extrinsic-mortality sweep applied to A, used only
  for C11.3.

Criteria:

- [ ] **C11.1 Allometry is what it claims.** Measured basal metabolic rate
...
- [ ] **C11.2 Life-history tradeoff emerges.** Under A, a negative
      ...
      weaker.
- [ ] **C11.3 Senescence responds to extrinsic mortality.** Under `M-low`,
...
- [ ] **C11.4 Thermal preference becomes load-bearing.** Under A, the
...
- [ ] **C11.5 Ontogeny is real.** Juveniles are measurably constrained:
...
- [ ] **C11.6 Exactness and determinism.** Ledger exact to the milli-unit
      ...
      equality; physiology-disabled configs reproduce the Phase 10 fixture
      exactly.
- [ ] **C11.7 Disease, if enabled.** Contact-structured spread produces an
...
- Statistical: C11.1 through C11.4 as automated tests with recorded
...
- Disabled-section equality against the Phase 10 fixture.
...
Record explicitly: the per-organism cost delta against Phase 10, and the
...
| The realism criteria fail and it is unclear whether the mechanism or the world is wrong | Each criterion names the specific mechanism it tests, and C11.1 gates the rest: if allometry does not verify, nothing downstream is interpretable |
...
subset can be disabled. All disabled reproduces the Phase 10 fixture
exactly.
