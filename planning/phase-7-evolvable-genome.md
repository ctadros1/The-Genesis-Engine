# Phase 7: Evolvable Genome, Diploid Genetics And Variable Topology
...
- No plasticity. `PlasticityGenes` are carried, inherited, validated, and
  behaviorally inert until Phase 8, following exactly the precedent set by
  thermal preference and defense tendency in Phase 2.
- No new sensory or action capability. The registry is populated with
  exactly the channels that exist after Phase 6, so the phase measures the
...
- No SIMD or batching rework. Variable topology makes the existing batching
  ...
  a later performance slice, not a Phase 7 deliverable.

## Prerequisites

- Phase 5 (multi-seed harness) and Phase 6 (the channel set the registry
  will describe).
...
- [ ] **C7.1 Structural evolution actually occurs.** Under A, the median
...
- [ ] **C7.2 Structural freedom does not destabilize the ecology.** Median
...
- [ ] **C7.3 Mendelian validation.** Under condition C, at a marked neutral
...
- [ ] **C7.4 Linkage validation.** Under condition C, the association
...
- [ ] **C7.5 Duplication versus explicit insertion.** Report the structural
      growth rate and the C7.2 stability outcome separately for a
      ...
      produce C7.1 within the run budget, that is a finding and the
      insertion operator becomes the default with the reason recorded.
- [ ] **C7.6 Bounded and fail-closed.** A seeded malformed-input harness of
...
- [ ] **C7.7 Determinism and fixtures.** Clean-process replay of the Phase 7
...
- [ ] **C7.8 Snapshot budget.** Snapshot size per organism and checkpoint
...
- Genetics validation: C7.3 and C7.4 as automated statistical tests with
...
| Risk | Mitigation |
|---|---|
| Genome bloat: duplication grows genomes until snapshots and memory become unmanageable | Hard caps with deterministic rejection; caps set from the C7.8 measurement; deletion rate configured to be non-negligible; genome size is a reported metric with an alert threshold |
...
| Duplication-driven growth is too slow to show C7.1 in the run budget | C7.5 makes this an explicit measured comparison with a defined fallback rather than a discovered failure |
