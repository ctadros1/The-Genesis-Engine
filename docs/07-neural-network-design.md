...
## Planned Successor: `lifesim-controller-v2` (Phase 7)
...
only we can change. Phase 7 replaces it; the design is
...
- The four documented neutral placeholders below stop being placeholders.
  Health (2), threat (10), and recent damage (16) become live in Phase 6,
  and attack (4) stops being a no-op in the same phase. **Phase 6 needs no
...
- Plasticity (Phase 8) makes weights change within a lifetime under a
...
The "no evolving topology" part of this recommendation was correct for Phase
2 and is reversed for Phase 7 by ADR-0013, with the costs it identified
...
real and are accepted, measured, and mitigated in Phase 7 rather than
...
This strategy assumes shared topologies and does not survive Phase 7, where
topologies are per-organism. The replacement is a measurement question for a
later performance slice, not a Phase 7 deliverable. If the current
