...
| **Should `max_composition_breadth` mean leaf constituents rather than direct ones?** D-116 recorded that under binary combine every composite has exactly two direct constituents, validation refuses a breadth below two, and the `BreadthCap` refusal is reachable by no valid world - the field is vestigial. Reinterpreting it as the count of leaf constituents in the whole tree (a full depth-4 tree has 16) would give the cap something to bind on, but it is a behaviour change under Rule 9: a new `lifesim-artifact` policy version, a new config hash for every artifact world, and the trace fixture re-pinned. | Before any second artifact campaign or any non-binary combine. | Leave the field as it is, documented as vestigial; if a follow-up campaign wants a breadth cap, take (a) through the policy version and record the old and new fixture hashes. |
| **Should the C arm's `artifact.inert` let consumption and decay run?** D-118: the inert early return skips carcass consumption, so the 'actions fire, no effect' control also had less food than A (starvation +6%, damage deaths -42%). The intended control skips exactly the five verbs. | Before the next artifact campaign is pre-registered. | Fix it under a new `lifesim-artifact` policy version with the trace fixture re-pinned; until then every reading of the C arm carries D-118's caveat, and no verdict in `phase12-artifact-confirmatory-findings.txt` depends on it. |
