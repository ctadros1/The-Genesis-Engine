...
Rare configurable disasters are event-driven, scoped, and auditable. Examples: drought, wildfire, storm, cold snap, and food blight. Each has a seed, spatial extent, duration, intensity, and start/end event. The product-direction default is rare, bounded disasters enabled in the mature high-realism world profile. They remain disabled because the implementation does not exist yet, and they are not scheduled in Phases 5 through 18. A future disaster slice requires long-run recovery and extinction evidence first, and its own plan entry.
