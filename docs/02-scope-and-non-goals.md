...
- **Phase 8** Allometric metabolism, thermoregulation, senescence, non-food extrinsic mortality, juvenile mortality, and death-cause accounting, so that population is bounded by ecology rather than by the `max_entities` guard.
...
- **Phase 16** The multicellularity transition as a representation change, with multicellularity itself reached by ordinary morphological evolution.
- **Phase 17** Offline era and tradition detection over the event log.
- **Phase 18** Intra-world parallelism under thread-count invariance: cross-cutting infrastructure rather than a behavioral phase, raising the per-world population ceiling without which several later criteria are untestable.

## Explicitly Deferred

- Broad multi-body-plan ecosystems as an authored feature. Structural
  morphology becomes evolvable in Phase 10 (ADR-0019), but as a lattice of
...
- Molecular-level genetics: codons, transcription, protein folding, or gene regulatory dynamics at chemical timescales. The regulatory locus type reserved by schema 2 is allocated in Phase 10 for development, at a coarse level only.
- GPU inference, public multi-user access, PostgreSQL, and intra-world parallelism. Each requires its own benchmark-backed decision. Structural neural-topology evolution moves out of this list and into Phase 9 under ADR-0013.
...
- **Training neural networks with reinforcement learning against a hand-authored reward.** Lifetime learning is in scope from Phase 11, and this non-goal is sharpened rather than relaxed by it: there is no reward function, no fitness signal delivered to any network, and no gradient against an objective we chose. The signal that gates plasticity is an ordinary output of the organism's own evolved network. See ADR-0014.
...
`scratch` origin mode. It is scoped to Phases 15 and 16, it requires a
