...
(cd apps/console && exec ./node_modules/.bin/vite preview --host 127.0.0.1 --port "$ui_port") &
ui_pid=$!
...
dev_attempt=0
until curl --silent --fail "http://127.0.0.1:$rest_port/api/health" >/dev/null; do
...
echo "Server:  http://127.0.0.1:$rest_port  (WS ws://127.0.0.1:$ws_port)"
