...
| 21 | `BirthSite` **(shipped, tag 31, version 13)** | organism, cell, occupants already in that cell (every one ahead of it in the feeding order), the organism's own trait-derived maturity, the cell's four substrates (primordial, monomer, polymer, waste), microbial density and biomass as the newcomer finds them - one record per admission, births and materializations alike, read after the tick's feeding; observation only, ignored by reconciliation; 67 bytes fixed width |
