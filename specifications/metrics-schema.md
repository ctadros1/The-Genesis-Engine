...
| 16 | `lifesim_materialized_total`, `lifesim_materialized_milli`, `lifesim_transition_events_total`, `lifesim_transition_deferred_cap_total`, `lifesim_transition_deferred_capacity_total`, `lifesim_transition_refused_total`, `lifesim_max_modules`, `lifesim_multi_module_organisms` | counter, counter, counter, counter, counter, counter, gauge, gauge. *Status 2026-09-02: kernel-side in `MetricsSnapshot` (the `transition_*` block plus `max_modules` and `multi_module_organisms`), exported through the schema-12 fixture line and the `field-series 2` campaign series (four appended columns); `refused_total` is a bug signal and is asserted zero by the fixture; `sim-server` export follows the phase-12 note above* |
| 17 | `lifesim_chemistry_consumed_milli` | counter. *Status 2026-09-03: kernel-side in `MetricsSnapshot` (`chemistry_consumed_milli`, the gross substrate taken by organisms, ADR-0034), exported through the schema-13 fixture line and the `field-series 3` campaign series (one appended column); zero whenever `chemistry.consumption_fraction_q16` is zero* |
