...
- **Duplication-driven growth is slower and less directed than explicit
  ...
  operator remains available as a configured variation policy, and Phase 9
  criterion C9.5 measures the comparison rather than assuming an answer. If
...
No performance claim is made here. Phase 9's benchmark requirements demand
...
- C9.5 shows duplication-only growth cannot reach the structural change the
...
- Phase 9 acceptance criteria C9.1 through C9.8, in particular the Mendelian
  and linkage validations (C9.3, C9.4) and the duplication-versus-insertion
  comparison (C9.5).
